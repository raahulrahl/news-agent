...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting news-agent in development mode..."
	@uv run watchfiles "python -m news_agent" news_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t news_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t news_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi news_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag news_agent:latest para5/news_agent:latest
	@docker push para5/news_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep news_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep news_agent || echo "No images found"
	@echo "\nAgent Health:"
